...
void GCS_MAVLINK::send_battery_status(const uint8_t instance) const
{
    ...
    if (battery.has_cell_voltages(instance)) {
        ...
        */
...
#if OSD_ENABLED
        int8_t cell_count = AP::osd()->cell_count;
        if (cell_count <= 1) {
            cell_count = 0;
        } else if (cell_count > (MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN + MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_EXT_LEN)) {
            cell_count = MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN + MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_EXT_LEN;
        }
#else
        const int8_t cell_count = 0;
#endif        
        if (cell_count) {
            voltage_mV /= cell_count;
        }
        for (uint8_t i = 0; i < MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN; i++) {
          if (cell_count ){
            cell_mvolts[i] = i < cell_count ? voltage_mV : UINT16_MAX;
          } else if (voltage_mV < 0.001f) {
        ...
        for (uint8_t i = 0; i < MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_EXT_LEN; i++) {
            if (cell_count && (i + MAVLINK_MSG_BATTERY_STATUS_FIELD_VOLTAGES_LEN) < cell_count) {
                cell_mvolts_ext[i] = voltage_mV;
            } else {
                // no more cells, set to zero
                cell_mvolts_ext[i] = 0;
            }
        }
    }
...
void GCS_MAVLINK::send_rc_channels_raw() const
{
    ...
    // implementations
    const bool send_rc = rc().has_had_rc_receiver();
    if (!(sending_mavlink1() || send_rc)) {
        return;
    }
    ...
    mavlink_msg_rc_channels_raw_send(
        ...
        AP_HAL::millis(),
        0,
        send_rc ? hal.rcin->read(0) : values[0],
        send_rc ? hal.rcin->read(1) : values[1],
        send_rc ? hal.rcin->read(2) : values[2],
        send_rc ? hal.rcin->read(3) : values[3],
        values[4],
        values[5],
        values[6],
        values[7],
